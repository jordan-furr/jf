...
export const POSTS_QUERY = defineQuery(`*[_type == "post" && defined(slug.current) && !("letter" in categories[]->slug.current)]|order(publishedAt desc)[0...100]{
...
export const ARIZONA_POSTS_QUERY = defineQuery(`*[_type == "post" && defined(slug.current) && "southwest" in categories[]->slug.current]|order(publishedAt asc)[0...100]{
  _id, title, slug, publishedAt, location, body
}`)
